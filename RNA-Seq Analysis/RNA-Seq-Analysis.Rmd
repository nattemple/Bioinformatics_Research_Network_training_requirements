...
# 'IL-1ß and SERPINA3 are markers of an aggressive
# Barretts Oesophagus phenotype identified using mRNA sequencing'
study <- "Barretts"
```
...
rse <-
  subset(rse,
    select = colData(rse)$cond %in%
      c("Simple_intestinal_metaplasia"
        , "High_Grade_Dysplasia")
  )
colData(rse) <- as.data.frame(colData(rse)) %>%
